...
function 登录(Account,Password)
	...
	while true do
		...
		if findstr("图像验证") == true then
			if findPicAndClick("登录邮箱",true,150,10)==true or findStrSub("邮箱地址",true) then
				sleep(1000)
				setIme(true)
				inputText(Account,true)
				sleep(1000)
				if findPicAndClick("登录密码",true)==true or findStrSub("密码",true) then
					sleep(1000)
					...
					if 过滑块验证(10)==true then
						...
						if findpic("登入",true) == true or findStrSub("登入",true) then
							sleep(2000)
